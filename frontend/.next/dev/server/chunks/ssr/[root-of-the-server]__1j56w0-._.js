...
function Home() {
    ...
    const buildGuide = ()=>{
        ...
        const steps = images.length ? images.map((img, i)=>`Step ${i + 1}: Refer to screenshot "${img.name}". Follow the on-screen prompts and confirm your action before proceeding.`) : [
    ...
    const handleGenerate = async ()=>{
        if (!discipline || !environment) {
            __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$nextjs$2d$toast$2d$notify$2f$dist$2f$nextjs$2d$toast$2d$notify$2e$es$2e$min$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["showToast"].error("Please select a discipline and a work environment.");
            return;
        }
        setLoading(true);
        try {
    ...
    const downloadWord = ()=>{
        if (!generated) return;
        const bodyHtml = generated.split("\n").map((line)=>line ? `<p>${escapeHtml(line)}</p>` : "<p>&nbsp;</p>").join("");
        const imagesHtml = images.map((img)=>`<h3>${escapeHtml(img.name)}</h3><img src="${img.dataUrl}" style="max-width:600px;" />`).join("");
        ...
        a.href = url;
        a.download = `work-instruction-${discipline}-${environment}.doc`.replace(/\s+/g, "-").toLowerCase();
        a.click();
